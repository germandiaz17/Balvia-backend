...
type settingsResponse struct {
	...
	// AppliesToNextPeriod is true whenever the change was deferred, which is the
	// normal case under domain rule 8. It is false only when the onboarding
	// carve-out reshaped the active period in place. Sent explicitly so the
	// client does not have to hardcode the rule.
...
type updateSettingsRequest struct {
	...
	TrackingPeriodMode   *string `json:"tracking_period_mode" validate:"omitempty,oneof=rolling calendar_month"`
...
func toSettingsResponse(v services.SettingsView) settingsResponse {
	...
	resp := settingsResponse{
		...
		TrackingPeriodMode:   s.TrackingPeriodMode,
		...
		AppliesToNextPeriod:  !v.ReshapedActivePeriod,
	}
...
func (h *UserSettingsHandler) Update(c *fiber.Ctx) error {
	...
	view, err := h.svc.Update(c.Context(), userID, services.SettingsUpdateInput{
		...
		TrackingPeriodMode:   req.TrackingPeriodMode,
